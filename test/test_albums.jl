...
@testset verbose = true "GET-request endpoints for albums" begin
    ...
    @testset "For market = $(market_id)" for market_id in markets
        ...
        @test ~isempty(Spotify.album_get(album_id, market = market_id )[1])
        @test ~isempty(Spotify.album_get_tracks(album_id, market = market_id)[1])

    end

    @testset "For offset = $(num_offset)" for num_offset in offsets

        @test ~isempty(Spotify.album_get_tracks(album_id, offset = num_offset)[1])
