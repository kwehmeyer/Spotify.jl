...
@testset verbose = true "GET-request endpoints for browse categories" begin
    ...
    @testset "For country = $(country_id)" for country_id in countries
        ...
        @test ~isempty(Spotify.category_get_single(category_id, country = country_id)[1])

        @testset "For offset = $(num_offset)" for num_offset in offsets

            @test ~isempty(Spotify.category_get_multiple(country = country_id, offset = num_offset)[1])
            @test ~isempty(Spotify.category_get_new_releases(country = country_id, offset = num_offset)[1])
    ...
    @test ~isempty(Spotify.recommendations_get(seeds, track_attributes = track_attributes)[1])
