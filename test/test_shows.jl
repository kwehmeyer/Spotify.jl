...
@testset verbose = true "GET-request endpoints for shows" begin
    ...
    @testset "For market = $(market_id)" for market_id in markets
        ...
        @test ~isempty(Spotify.show_get_single(show_id, market = "$market_id")[1])

        @test ~isempty(Spotify.show_get_multiple("$show_id, 4rOoJ6Egrf8K2IrywzwOMk", market = "$market_id")[1])

        @test ~isempty(Spotify.show_get_episodes(show_id, market = "$market_id")[1])

    end

    @test ~isempty(Spotify.show_get_saved()[1])
